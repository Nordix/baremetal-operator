...
func (r *BareMetalHostReconciler) retrievePreprovisioningExtraKernelParamsSpec(info *reconcileInfo, preprovImg *provisioner.PreprovisioningImage) string {
	...
	if preprovImg != nil {
		...
		case metal3api.ImageFormatInitRD:
			trimmedImageParams := strings.TrimSpace(preprovImg.GeneratedImage.ExtraKernelParams)
			if trimmedImageParams != "" {
				kernelExtraPreprovParams += " " + trimmedImageParams
			}
...
func (r *BareMetalHostReconciler) registerHost(ctx context.Context, prov provisioner.Provisioner, info *reconcileInfo) actionResult {
	...
	// Nil value is acceptable it means the provisioner doesn't support PPI.
	...
	provResult, provID, err := prov.Register(
		...
		provisioner.ManagementAccessData{
			...
			PreprovisioningExtraKernelParams: r.retrievePreprovisioningExtraKernelParamsSpec(info, preprovImg),
